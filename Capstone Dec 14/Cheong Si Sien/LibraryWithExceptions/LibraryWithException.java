...
public class LibraryWithException {
    ...
    static ArrayList<BookWithExceptions> arr = new ArrayList<BookWithExceptions>(); // Book as ArrayList type
        ...
        Scanner sc = new Scanner(System.in);         
        {
            while (true) {
                try {
                    ...
                    sc.next();
                    System.out.println("\033[0;31mBook ID must be a number");
                }
            }
        }
...

        {
            while (true) {
                ...
                //} catch (InputMismatchException ie) {
                    ...
                    sc.next();
                    System.out.println("\033[0;31mBorrower ID must be a number");
                }    
            }
        }
            ...
            author = sc.next();
            arr.add(new BookWithExceptions(id, title, borrowerid, author));
            display();
        ...
        if (arr.size() != 0) {
            ...
            for (int i = 0; i < arr.size(); i++) {
                if (arr.get(i).getId() == id) {
                    ...
                    arr.set(i, new BookWithExceptions(id, title, borrowerid, author));
                    j++;
                    break;
                }
            }
